...
class MediaSettingsParserBase(ABC):
    ...
    def _get_lane_values_str(val_dict, lane_count, subport_num=0):
        """Slice per-lane values and return as a comma-separated string.
    ...
    def to_db_value(media_dict, lane_count, subport_num, gearbox_line_lane_count=None):
        """Convert traditional media settings to a list of (key, value_str) pairs for APP DB.

        Args:
            media_dict: resolved media settings dictionary
            lane_count: number of lanes for this logical port
            subport_num: subport number (1-based), 0 for non-breakout case
            gearbox_line_lane_count: lane count on the gearbox line-side, or None

        Returns:
            list of (key, value_string) tuples ready for APP DB
        """
        fvs = []
        for media_key in media_dict:
            if type(media_dict[media_key]) is dict:
                if "gb_line" in media_key and gearbox_line_lane_count is not None:
                    lane_count_si = gearbox_line_lane_count
                else:
                    lane_count_si = lane_count
                val_str = MediaSettingsParserBase._get_lane_values_str(
                    media_dict[media_key], lane_count_si, subport_num)
            else:
                val_str = media_dict[media_key]
            fvs.append((str(media_key), str(val_str)))
        return fvs
...
class CustomMediaSettingsParser(MediaSettingsParserBase):
    ...
    def is_port_selected(port_selector, physical_port):
        ...
        for token in port_selector.split(CustomMediaSettingsParser.COMMA_SEPARATOR):
            token = token.strip()
            if not token:
                helper_logger.log_notice(
            ...
            if CustomMediaSettingsParser.RANGE_SEPARATOR in token:
                start_str, end_str = token.split(CustomMediaSettingsParser.RANGE_SEPARATOR, 1)
            else:
    ...
    def _get_lane_values(val_dict, lane_count, subport_num):
        """Slice per-lane values as a list (preserving original types)."""
        start_lane_idx = (subport_num - 1) * lane_count if subport_num else 0
        val_list = [val_dict[lane_key] for lane_key in natsorted(val_dict)]
    ...
    def to_db_value(custom_media_dict, lane_count, subport_num):
        ...
        Args:
            custom_media_dict: dictionary containing custom SerDes settings
            lane_count: number of lanes for this subport
            subport_num: subport number (1-based), 0 for non-breakout case
...
def get_custom_media_settings_value(physical_port, key):
    ...
    Returns the raw custom-attribute dictionary (with CUSTOM: keys) before
    it is converted to the APP_DB custom_serdes_attrs payload.
    Returns {} if no custom settings match.
    """
...
def notify_media_setting(logical_port_name, transceiver_dict,
    ...
    for physical_port in physical_port_list:
        ...
        # Custom settings: serialize to a single JSON payload
        custom_db_value = CustomMediaSettingsParser.to_db_value(custom_media_dict, lane_count, subport_num)
